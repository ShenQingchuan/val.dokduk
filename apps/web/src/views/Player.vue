...
const translatedCurrentRank = computed(() =>
  getTranslatedRank(t, playerStore.mmr?.current_data?.currenttierpatched),
)
const translatedPeakRank = computed(() =>
  getTranslatedRank(t, playerStore.mmr?.highest_rank?.patched_tier),
)
const peakRankIconUrl = computed(() =>
  getRankIconUrl(playerStore.mmr?.highest_rank?.tier),
)
const peakRankSeason = computed(() =>
  formatSeason(t, playerStore.mmr?.highest_rank?.season),
)
...
// 使用后端计算的统计数据
const playerStats = computed(() => playerStore.stats)
...
const rowVirtualizer = useWindowVirtualizer(computed(() => ({
  count: playerStore.matches.length,
  estimateSize: () => 145,
  overscan: 5,
...
function handleScroll() {
  ...
  if (scrollHeight - scrollTop - clientHeight < 200) {
    playerStore.loadMoreMatches('ap', name.value, tag.value)
  }
}

onMounted(() => {
  if (name.value && tag.value) {
    playerStore.fetchPlayerWithMatches(name.value, tag.value)
  }
...
watch([name, tag], ([newName, newTag]) => {
  if (newName && newTag) {
    playerStore.fetchPlayerWithMatches(newName, newTag)
  }
})
</script>
...
<template>
  <div class="min-h-screen">
    <!-- Loading state -->
    <div v-if="playerStore.playerLoading" class="flex items-center justify-center py-32">
    ...
    <!-- Error state -->
    <div v-else-if="playerStore.playerError" class="px-4 py-32">
      <div class="text-center max-w-md mx-auto">
        ...
        <p class="text-val-red mb-6">
          {{ playerStore.playerError }}
        </p>
        <button
          class="btn-val rounded-lg"
          @click="playerStore.fetchPlayerWithMatches(name, tag)"
        >
          {{ t('common_retry') }}
        </button>
      </div>
    ...
    <!-- Player data -->
    <div v-else-if="playerStore.account" class="pb-8">
      <!-- Hero Section: Player Card Banner -->
      <div class="relative">
        <!-- Background image -->
        <div class="h-30 md:h-48 relative overflow-hidden">
          <img
            v-if="playerStore.account.card?.wide"
            :src="playerStore.account.card.wide"
            alt="Player Card"
      ...
      <div class="max-w-4xl mx-auto px-4 md:px-8 mt-6">
        ...
        <div class="grid grid-cols-2 gap-3 mb-4">
          ...
          <div class="card-val p-4">
            ...
            <div v-if="playerStore.mmr" class="flex items-center gap-3">
              <div class="w-14 h-14 flex-shrink-0 flex items-center justify-center">
                <img
                  v-if="playerStore.mmr.current_data?.images?.large"
                  :src="playerStore.mmr.current_data.images.large"
                  :alt="playerStore.mmr.current_data.currenttierpatched"
              ...
              <div>
                <p class="text-lg font-medium text-val-cream">
                  {{ translatedCurrentRank }}
                </p>
                <p class="text-val-gray text-sm">
                  {{ playerStore.mmr.current_data?.ranking_in_tier || 0 }} {{ t('player_rr') }}
                </p>
              </div>
            </div>
          ...
          <div class="card-val p-4">
            ...
            <div v-if="playerStore.mmr?.highest_rank" class="flex items-center gap-3">
              <div class="w-14 h-14 flex-shrink-0 flex items-center justify-center">
                <img
                  v-if="peakRankIconUrl"
                  :src="peakRankIconUrl"
                  :alt="playerStore.mmr.highest_rank.patched_tier"
                  class="w-full h-full object-contain drop-shadow-[0_0_10px_rgba(255,70,85,0.3)]"
                >
                <div v-else class="i-simple-icons-valorant w-10 h-10 text-val-gray-dark" />
              </div>
        ...
        <!-- Stats Grid -->
        <div v-if="playerStats && playerStats.totalGames > 0" class="mb-6">
          <h3 class="text-xs font-medium text-val-gray uppercase tracking-wider mb-3">
            {{ t('stats_recent') }} {{ playerStats.totalGames }} {{ t('stats_games') }}
          </h3>

          <div class="grid grid-cols-4 md:grid-cols-4 gap-2">
            <!-- 胜率 -->
            <div class="card-val p-2.5">
              <p class="text-xs text-val-gray mb-0.5">
                {{ t('stats_win_rate') }}
              </p>
              <p class="text-lg font-medium text-val-cream">
                {{ formatPercent(playerStats.winRate) }}
              </p>
              <p class="text-xs text-val-gray">
                {{ playerStats.wins }}W {{ playerStats.totalGames - playerStats.wins }}L
              </p>
            </div>
            ...
            <div class="card-val p-2.5">
              ...
              <p class="text-lg font-medium" :class="playerStats.kd >= 1 ? 'text-green-400' : 'text-val-red'">
                {{ formatStat(playerStats.kd, 2) }}
              </p>
              <p class="text-xs text-val-gray">
            ...
            <div class="card-val p-2.5">
              ...
              <p class="text-lg font-medium text-val-cream">
                {{ formatPercent(playerStats.headshotRate) }}
              </p>
              <p class="text-xs text-val-gray">
                {{ playerStats.totalHeadshots }} HS
              </p>
            </div>
            ...
            <div class="card-val p-2.5">
              ...
              <p class="text-lg font-medium text-val-cream">
                {{ formatPercent(playerStats.kast) }}
              </p>
            </div>
            ...
            <div class="card-val p-2.5">
              ...
              <p class="text-lg font-medium" :class="playerStats.damageDeltaPerRound >= 0 ? 'text-green-400' : 'text-val-red'">
                {{ playerStats.damageDeltaPerRound >= 0 ? '+' : '' }}{{ formatStat(playerStats.damageDeltaPerRound, 1) }}
              </p>
            </div>
            ...
            <div class="card-val p-2.5">
              ...
              <p class="text-lg font-medium text-val-cream">
                {{ formatStat(playerStats.killsPerRound, 2) }}
              </p>
            </div>
        ...
        <!-- Stats loading -->
        <div v-else-if="playerStore.matchesLoading" class="mb-6 text-center py-8">
        ...
        <div ref="listContainerRef" class="mt-6">
          <h3 class="font-medium text-val-gray uppercase mb-2">
            {{ t('player_matches') }}
          </h3>

          <!-- Matches loading -->
          <div v-if="playerStore.matchesLoading && playerStore.matches.length === 0" class="flex items-center justify-center py-20">
            <div class="text-center">
          ...
          <!-- Matches error -->
          <div v-else-if="playerStore.matchesError && playerStore.matches.length === 0" class="text-center py-20">
          ...
          <!-- Empty state -->
          <div v-else-if="playerStore.matches.length === 0 && !playerStore.matchesLoading" class="text-center py-20">
          ...
          <div
            v-else-if="playerStore.matches.length > 0"
          ...
          >
            ...
            >
              <div class="py-1.5">
                <MatchCardFull
                  :match="playerStore.matches[virtualRow.index]!"
                  :player-name="name"
                  :player-tag="tag"
                />
              </div>
          ...
          <!-- Load more indicator -->
          <div v-if="playerStore.matchesLoadingMore" class="flex justify-center py-6">
            <div class="i-svg-spinners-ring-resize w-8 h-8 text-val-red" />
          </div>

          <!-- No more data -->
          <div v-else-if="!playerStore.matchesHasMore && playerStore.matches.length > 0" class="text-center py-4">
            <p class="text-val-gray text-sm">
              {{ t('match_no_more') }}
            </p>
          </div>
